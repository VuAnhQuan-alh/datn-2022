...
const TestCase = ({ data }) => {
  ...
  const { data: dataTest, status } = useSelector(
  ...
  const test_cases = data.test_case?.map((item, idx) => ({
    key: idx,
    ...item,
  }));
    ...
    return (
      <>
        <Row>
          ...
          <Col span={16}>{test?.input.map((el) => el.value)}</Col>
        </Row>
        <Row>
          <Col span={8}>Actual output:</Col>
          <Col span={16}>
            {!isEmpty(equalTest) &&
              equalTest.test_case?.find((item) => item.id === test._id)?.output}
          </Col>
        </Row>
        <Row>
        ...
        <Row>
          ...
          <Col span={16}>
            {!isEmpty(equalTest) &&
              equalTest.test_case?.find((item) => item.id === test._id)
                ?.time_run}
          </Col>
        </Row>
        <Row>
          ...
          <Col span={16}>
            {!isEmpty(equalTest) &&
              equalTest.test_case?.find((item) => item.id === test._id)?.status}
          </Col>
        </Row>
      </>
    );
          ...
          <TabPane
            ...
            tab={
              <Row align="middle">
                ...
                {isEmpty(equalTest) ? (
                  ""
                ) : equalTest.test_case?.find((test) => test.id === item._id)
                    ?.pass ? (
