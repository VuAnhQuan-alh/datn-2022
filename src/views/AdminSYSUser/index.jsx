...
import { Switch, Button, Popconfirm, Card, Col, Row, Table, Avatar, Image } from 'antd'
      ...
      .then(response => {
        if (response.status === 200) {
          const result = response.data.data?.map((item, idx) => {
            ...
            return { stt: ++idx, key: idx, avatar, email, name, rank, score, _id, verified }
          })
          setDataUser(result)
        }
      })
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => <Row gutter={8}>
        <Col>
          <Avatar src={<Image src={record?.avatar || `https://joeschmoe.io/api/v1/${record.stt}`} alt="icon user" />} size={36} />
        </Col>
        <Col>
          {record.name}
        </Col>
    ...
    },
    {
      ...
      key: 'action',
      align: 'center',
      render: (_, record) => <Row gutter={16} justify="center" align='middle'>
        <Col>
