...
const CardChallenge = ({ data }) => {
  return (
    <>
      {data.length &&
        data?.map((item, idx) => (
          <Card key={idx} className="custom-card-challenge">
            <Row gutter={16}>
              <Col>
                <Avatar
                  src={
                    <Image src={`https://joeschmoe.io/api/v1/random${idx}`} />
                  }
                  size={72}
                />
              </Col>
              ...
              <Col flex="auto" align="end">
                <CustomButton
                  text="Giải bài này"
                  href={`/challenge/${item?._id}/detail`}
                />
              </Col>
            </Row>
            <Divider
              style={{ margin: "12px 0", borderTop: "2px solid #0000001a" }}
            />
